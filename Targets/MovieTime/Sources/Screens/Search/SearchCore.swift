    ...
    var body: some ReducerProtocol<State, Action> {
        Reduce { state, action in
            ...
            case let .search(term):
                ...
                return movieService.search(query: term, page: state.currentPage)
            ...
            case .loadMore:
                ...
                state.loadingMoreFailed = false
                return movieService.search(query: state.searchTerm, page: state.currentPage)
                    .receive(on: mainQueue)
                    .catchToEffect()
        ...
        .forEach(\.movieStates, action: /Search.Action.movie(movieId:action:)) {
            MovieListEntry()
        }
    }
}
